...
- `backend/`：Go 模块，**module 名是 `github.com/jiftle/adminbase`**。新增 Go 文件的 import 必须写 `github.com/jiftle/adminbase/...`。
...
- 生成后确认 `internal/dao/*.go` 的 import 为 `github.com/jiftle/adminbase/internal/dao/internal`；gf 不覆盖已存在的外部 dao 文件，必要时删除后重生成。
